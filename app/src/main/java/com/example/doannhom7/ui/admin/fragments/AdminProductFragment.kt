...
import com.example.doannhom7.ui.admin.AdminProductFormActivity
import com.example.doannhom7.ui.admin.adapters.ProductsAdapter
import com.google.android.material.floatingactionbutton.FloatingActionButton
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext

class AdminProductFragment : Fragment(R.layout.fragment_admin_products) {
    ...
    private fun setControl(view: View) {
        ...
        adapter = ProductsAdapter(
            data = emptyList(),
            onClick = { p ->
                val i = Intent(requireContext(), AdminProductFormActivity::class.java)
                i.putExtra("productId", p.id)
                startActivity(i)
            },
            onLongClick = { p -> askDelete(p) }
        )
    ...
    private fun setEvent() {
        fabAdd.setOnClickListener {
            startActivity(Intent(requireContext(), AdminProductFormActivity::class.java))
        }
    ...
    private fun search(q: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            val list = db.productDao().search("%$q%")
            withContext(Dispatchers.Main) { adapter.submit(list) }
        }
    }

    private fun askDelete(p: ProductEntity) {
        android.app.AlertDialog.Builder(requireContext())
            .setTitle("Xoá sản phẩm?")
            .setMessage("Xoá: ${p.name} (${p.code}) ?")
